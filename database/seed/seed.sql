...
ON CONFLICT (github_id) DO NOTHING;
INSERT INTO issues (github_id, repository_id, number, title, body, url, difficulty_score, difficulty, required_skills, technologies, learning_tags, comments)
SELECT 999002, id, 1, 'Implement feature enhancements and test pull request flow', 'Add test implementation for listtty and submit pull request.', 'https://github.com/dizziedbliss/listtty/issues/1', 3, 'BEGINNER', ARRAY['Python', 'CLI'], ARRAY['Python', 'FastAPI'], ARRAY['beginner-friendly', 'testing'], 4 FROM repositories WHERE github_id = 999001
ON CONFLICT (github_id) DO NOTHING;
