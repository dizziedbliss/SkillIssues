...
import { ArrowLeft, Bookmark, Check, ExternalLink, Filter, MessageCircle, Pencil, Search, X } from 'lucide-react';
import './styles.css';

type Issue = { id: number; number: number; title: string; body: string; difficulty: string; difficulty_score: number; required_skills: string[]; technologies: string[]; repository: string; language: string; comments: number; url: string; updated_at?: string };
type Profile = { username: string; avatar_url: string; bio: string; target_level: string; interests: string[]; preferred_languages: string[]; demonstrated_skills: string[]; progress_score: number; completed_count: number };
type Contribution = { id: number; issue_id: number; number: number; title: string; repository: string; language: string; state: string; pr_url: string | null; created_at: string; issue_url: string };
type View = 'SkillIssues' | 'Saved' | 'Working' | 'Submissions';

const API = import.meta.env.VITE_API_URL || 'http://localhost:8000';
const languageIcons: Record<string, string> = { Python: 'python', TypeScript: 'typescript', JavaScript: 'javascript', HTML: 'html5', CSS: 'css3' };
const languageLogoBase = 'https://raw.githubusercontent.com/abranhe/programming-languages-logos/master/src';
...
function App() {
  const [profile, setProfile] = useState<Profile | null>(null); const [issues, setIssues] = useState<Issue[]>([]); const [savedIssues, setSavedIssues] = useState<Issue[]>([]); const [working, setWorking] = useState<Contribution[]>([]); const [submissions, setSubmissions] = useState<Contribution[]>([]); const [view, setView] = useState<View>('SkillIssues'); const [selected, setSelected] = useState<Issue | null>(null); const [query, setQuery] = useState(''); const [error, setError] = useState(''); const [loginRequired, setLoginRequired] = useState(false); const [editing, setEditing] = useState(false); const [bio, setBio] = useState(''); const [languages, setLanguages] = useState(''); const [targetLevel, setTargetLevel] = useState('INTERMEDIATE');
  async function refresh() { try { const [user, personalized, saved, current, submitted] = await Promise.all([request<Profile>('/me'), request<Issue[]>('/recommendations?limit=20'), request<Issue[]>('/saved'), request<Contribution[]>('/working'), request<Contribution[]>('/contributions')]); setProfile(user); setBio(user.bio || ''); setLanguages(user.preferred_languages.join(', ')); setTargetLevel(user.target_level); setIssues(personalized); setSavedIssues(saved); setWorking(current); setSubmissions(submitted.filter((item) => item.state !== 'STARTED')); } catch (caught) { if (caught instanceof Error && caught.message === 'Authentication required') setLoginRequired(true); else setError('Start the API with docker compose up --build.'); } }
  async function search() { try { setIssues(await request<Issue[]>(`/issues?limit=20&search=${encodeURIComponent(query)}`)); } catch { setError('Search failed.'); } }
  useEffect(() => { void refresh(); }, []);
  async function loginDemo() { await request('/auth/demo', { method: 'POST' }); setLoginRequired(false); await refresh(); }
  async function loginGithub() { const result = await request<{ enabled: boolean; authorize_url?: string; message?: string }>('/auth/github'); if (result.enabled && result.authorize_url) window.location.assign(result.authorize_url); else setError(result.message || 'GitHub login is not configured.'); }
  async function toggleSave(issue: Issue) { const isSaved = savedIssues.some((item) => item.id === issue.id); if (isSaved) { await request(`/saved/${issue.id}`, { method: 'DELETE' }); setSavedIssues((current) => current.filter((item) => item.id !== issue.id)); } else { await request('/saved', { method: 'POST', body: JSON.stringify({ issue_id: issue.id }) }); setSavedIssues((current) => [...current, issue]); } }
...
function SubmissionList({ items }: { items: Contribution[] }) { return <div className="issue-feed">{items.length ? items.map((item) => <article className="web-issue submission-card" key={item.id}><div className="issue-main"><div><h2>#{item.number} {item.title}</h2><p>{item.repository} · {item.language}</p></div><span className={`submission-label ${item.state === 'PR_MERGED' ? 'accepted' : item.state === 'PR_CLOSED' ? 'rejected' : 'review'}`}>{item.state === 'PR_MERGED' ? 'Accepted' : item.state === 'PR_CLOSED' ? 'Rejected' : 'Review required'}</span></div><div className="issue-meta"><span className="atom">Submitted {daysAgo(item.created_at)}</span>{item.pr_url && <a className="atom action" href={item.pr_url} target="_blank" rel="noreferrer">View submission <ExternalLink size={11} /></a>}<a className="atom" href={item.issue_url} target="_blank" rel="noreferrer">View issue</a></div></article>) : <p className="empty-feed">Submit a pull request to see it here.</p>}</div>; }
